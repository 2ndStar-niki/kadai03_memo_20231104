...
document.getElementById("am_bm").addEventListener("click", function() {
    ...
    if (bookmarks.includes(bookmarkedText)) {
        ...
        this.src = "imgs/ブックマーク未.jpeg"; // 通常の画像に切り替え
        alert("お気に入りから削除しました");
    } else {
        // 特定の文字がブックマークされていない場合、追加
        bm_status = 1;
        bookmarks.push(bookmarkedText);
        this.src = "imgs/ブックマーク完.jpeg"; // ブックマーク済みの画像に切り替え
        alert("お気に入りに追加しました"); 
    }
...
document.getElementById('sm_bm').addEventListener('click', function() {
    ...
    if (bookmarks.includes(bookmarkedText)) {
        bookmarks = bookmarks.filter(bookmark => bookmark !== bookmarkedText);
        this.src = 'imgs/ブックマーク未.jpeg';
        alert('お気に入りから削除しました');
    } else {
        bookmarks.push(bookmarkedText);
        this.src = 'imgs/ブックマーク完.jpeg';
        alert('お気に入りに追加しました'); 
    }
...
$("#clear").on("click", function(){
    ...
    $("#bm_list").empty();
    // それぞれをブックマーク未の画像に変更
    $('#am_bm').attr('src', "imgs/ブックマーク未.jpeg");
    $('#sm_bm').attr('src', "imgs/ブックマーク未.jpeg");
    $('#km_bm').attr('src', "imgs/ブックマーク未.jpeg");
    $('#bm_bm').attr('src', "imgs/ブックマーク未.jpeg");
    $('#dc_bm').attr('src', "imgs/ブックマーク未.jpeg");
    $('#kr_bm').attr('src', "imgs/ブックマーク未.jpeg");
});
